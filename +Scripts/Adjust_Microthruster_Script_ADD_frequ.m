...
frequ_range = [1 10 50 100];
for kk = 1:length(frequ_range)
    for jj = 1:length(frequ_range)
...
sun_meas_frequ = frequ_range(jj);%frequ_range(jj); % sun sensor measurement frequency in Hz
mag_meas_frequ = 1; % magnetometer measurement frequ in Hz
gyro_meas_frequ = frequ_range(kk);%frequ_range(kk); % gyroscope measurement frequ in Hz
...
%     omega_b_b_1 = w_b_ECI(i-1,:)';
%     quat_b_ECI_1 = quat_b_ECI(i-1,:)';
%     C_b_ECI_1 = Functions.quat2rot(quat_b_ECI_1);
%     Lbb_1 = C_b_ECI_1*(J*omega_b_b_1);
%     Lbb_2 = C_b_ECI_2*(J*omega_b_b_2);
%     Tbb_true(:,i-1) = (Lbb_2-Lbb_1)./(t(i)-t(i-1));
%     Tbb_true_norms(i-1) = norm(Tbb_true(:,i-1));
end
...
% omega_b_b_2 = w_b_ECI(end,:)';
% omega_b_b_1 = w_b_ECI(end-1,:)';
figure
...
% Cb_ECI_1 = Functions.quat2rot(quat_b_ECI(end-1,:)');
% Cb_ECI_2 = Functions.quat2rot(quat_b_ECI(end,:)');

% J_1 = Cb_ECI_1*J;
% J_2 = Cb_ECI_2*J;

% Lbb_one_1 = J*omega_b_b_1;
% Lbb_one_2 = J*omega_b_b_2;
% Tbb_true_one = (Lbb_one_2-Lbb_one_1)./(t(end)-t(end-1));
...
for i = 2:length(t1)
%     domega = domega_b_ECI_ekfilt(i,:)'; % change in angular velocity
%     omega_b_b_1 = w_b_ECI_ekfilt(i,:)'; % angular velocity at that timestep
...
% T_uncert = norm(Tbb_true-mean(Tbb_EKF,2)); % vector uncertainty, 
% Luke Olson, Matt West, University of Illinois Urbana-Champaign
% thrust_meas_EKF = norm(mean(Tbb_EKF,2))/lever_arm_len; % Since torque 
% is now perpendicular to r, T = r*F
% r_thrust_uncert = T_uncert/lever_arm_len; % ROUGH calculation of thrust uncertainty
...
%     Lbb_1 = J*omega_b_b_1;
%     Lbb_2 = J*omega_b_b_2;
%     Tbb_UKF(:,i) = (Lbb_2-Lbb_1)./delta_t;
end
% thrust_meas_UKF = norm(mean(Tbb_UKF,2))/lever_arm_len; % Since torque is now perpendicular to r, T = r*F
% % T_uncert = std(Tbb_UKF,0,2)/sqrt(length(Tbb_UKF)); % uncertainty in each x,y,z Tbb term
% T_uncert = norm(Tbb_true-mean(Tbb_UKF,2)); % vector uncertainty
% r_thrust_uncert = norm(T_uncert)/lever_arm_len; % ROUGH calculation of thrust uncertainty
...
% Try further post processing with moving average filter
%{ 
% Not much change
sm_w_b_ECI_ukfilt_1 = smooth(w_b_ECI_ukfilt(:,1));
sm_w_b_ECI_ukfilt_2 = smooth(w_b_ECI_ukfilt(:,2));
sm_w_b_ECI_ukfilt_3 = smooth(w_b_ECI_ukfilt(:,3));
f_sm_UKF = figure;
subplot(1,2,1)
plot(t,w_b_ECI(:,1),t,w_b_ECI(:,2),t,w_b_ECI(:,3))
hold on
...
xlabel('Time (s)')
ylabel('\omega (rad/s)')
title('Angular Velocity: True vs. Smoothed UKF')
plot(t1,sm_w_b_ECI_ukfilt_1,'--',t1,sm_w_b_ECI_ukfilt_2,'--',...
   t1,sm_w_b_ECI_ukfilt_3,'--')
legend('\omega_1','\omega_2','\omega_3','\omega_{UKF1}','\omega_{UKF2}',...
    '\omega_{UKF3}')
f_sm_UKF.Units = 'normalized';
f_sm_UKF.Position = [0.5 0.3 0.5 0.5];
%}
%% Plot orbit
r = outKF.r.Data;

% Gravity = outKF.Gravity.Data;
% grav_norm_max = 0;
% for i = 1:length(Gravity)
%     Grav_norm = norm(Gravity(i,:));
%     if Grav_norm > grav_norm_max
%         grav_norm_max = Grav_norm;
%     end
% end
% disp("Max Tgg:" + grav_norm_max)
% B = outKF.B.Data;
% B_norm_max = 0;
% for i = 1:length(B)
%     B_norm = norm(B(i,:));
%     if B_norm > B_norm_max
%         B_norm_max = B_norm;
%     end
% end
% disp("Max Magnetic:" + B_norm_max)
% Drag = outKF.Drag.Data;
% Drag_norm_max = 0;
% for i = 1:length(Drag)
%     Drag_norm = norm(Drag(i,:));
%     if Drag_norm > Drag_norm_max
%         Drag_norm_max = Drag_norm;
%     end
% end
% disp("Max Drag:" + Drag_norm_max)
% Solar = outKF.Solar.Data;
% Solar_norm_max = 0;
% for i = 1:length(Solar)
%     Solar_norm = norm(Solar(i,:));
%     if Solar_norm > Solar_norm_max
%         Solar_norm_max = Solar_norm;
%     end
% end
% disp("Max Solar:" + Solar_norm_max)
% Max Tgg over orbit is 4.185e-07, non-negligible. Include in analysis
...
for kk = 1:length(frequ_range)
    ...
    if thrust_mean_EKF(kk,jj)*1e6 < control_thrust_EKF
        fprintf('\nGyro Frequ %d, Sun Frequ. %d EKF Results reduced by %f %%\n',frequ_range(kk),frequ_range(jj),EKF_perc_reduc*100)
    end
    if thrust_mean_UKF(kk,jj)*1e6 < control_thrust_UKF
        fprintf('\nGyro Frequ %d, Sun Frequ. %d UKF Results reduced by %f %%\n',frequ_range(kk),frequ_range(jj),UKF_perc_reduc*100)
    end
    end
end

save('ADD_outKF_Full_Sim_Frequ_1_100')