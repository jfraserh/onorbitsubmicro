%% Sub-Microthruster Fusion Technique Simulation: Initial Quaternion Adjust
% Jonathan Hood, 5/3/2022
% Run full sim 100 times to see if there is a correlation between the
% initial quaternion and the final measured thrust.
...
quat_b_ECI0 = randn([4 1]); % initial quaternion from detumble
quat_b_ECI0 = quat_b_ECI0./norm(quat_b_ECI0);
...
% Find norm of each quaternion initial condition

% Evaluate correlation between quaternion initial and final thrust
% measurement
[~,P_q1_EKF] = corrcoef(q_b_arr(1,:),thrust_mean_EKF);
[~, P_q1_UKF] = corrcoef(q_b_arr(1,:),thrust_mean_UKF);

[~,P_q2_EKF] = corrcoef(q_b_arr(2,:),thrust_mean_EKF);
[~, P_q2_UKF] = corrcoef(q_b_arr(2,:),thrust_mean_UKF);

[~,P_q3_EKF] = corrcoef(q_b_arr(3,:),thrust_mean_EKF);
[~, P_q3_UKF] = corrcoef(q_b_arr(3,:),thrust_mean_UKF);

[~,P_q4_EKF] = corrcoef(q_b_arr(4,:),thrust_mean_EKF);
[~, P_q4_UKF] = corrcoef(q_b_arr(4,:),thrust_mean_UKF);



[EKF_qb1,EKF_indx_1] = sort(q_b_arr(1,:));
thrust_qb1_EKF = thrust_mean_EKF(EKF_indx_1);

[EKF_qb2,EKF_indx_2] = sort(q_b_arr(2,:));
thrust_qb2_EKF = thrust_mean_EKF(EKF_indx_2);

[EKF_qb3,EKF_indx_3] = sort(q_b_arr(3,:));
thrust_qb3_EKF = thrust_mean_EKF(EKF_indx_3);

[EKF_qb4,EKF_indx_4] = sort(q_b_arr(4,:));
thrust_qb4_EKF = thrust_mean_EKF(EKF_indx_4);

figure
scatter(EKF_qb1,thrust_qb1_EKF)
hold on
grid on
xlabel('\epsilon_1')
ylabel('Thrust (uN)')
title('Initial Quaternion \epsilon_1 vs. Measured Thrust')
exportgraphics(gcf,'Spacecraft_init_con_q1.png')

figure
scatter(EKF_qb2,thrust_qb2_EKF)
hold on
grid on
xlabel('\epsilon_2')
ylabel('Thrust (uN)')
title('Initial Quaternion \epsilon_2 vs. Measured Thrust')
exportgraphics(gcf,'Spacecraft_init_con_q2.png')

figure
scatter(EKF_qb3,thrust_qb3_EKF)
hold on
grid on
xlabel('\epsilon_3')
ylabel('Thrust (uN)')
title('Initial Quaternion \epsilon_3 vs. Measured Thrust')
exportgraphics(gcf,'Spacecraft_init_con_q3.png')

figure
scatter(EKF_qb4,thrust_qb4_EKF)
hold on
grid on
xlabel('\eta')
ylabel('Thrust (uN)')
title('Initial Quaternion \eta vs. Measured Thrust')
exportgraphics(gcf,'Spacecraft_init_con_eta.png')

save('Spacecraft_outKF_Full_Sim_Init_Con_Q')
