...
warning('off','all')
simtime_range = [200 400 600 800 1000 1600 2000 2400];

for kk = 1:length(simtime_range)
%     clear -regexp [^kk]

% INITIAL Proposal -- v1ef.0
...
cube_max_mass = 2;    % Maximum mass of a single cube in kg
...
% From sim, know Mag is  nonnegligible (1e-5); must model to at least 1e-10
...
% From sim, know GG is nonnegligible (1e-7); must model to at least 1e-10,
% adjust as needed
...
simtime = simtime_range(kk);       % desired simulation runtime in sec
...
thrust_uncert(kk) = norm_uncert./lever_arm_len +1.7e-10+(6e-10)/lever_arm_len; % Error in thrust

thrust_mean = norm(mean_torque_true)/lever_arm_len; % Find thrust

disp("Mean true thrust: " + thrust_mean*1e6 + " +/- " + thrust_uncert(kk)*1e6 + " N")
...
for i = 2:length(t1)
    ...
    % Use dynamic equation to calculate torque in ECI

    % Convert torque in ECI to body frame
...
thrust_uncert_EKF(kk) = norm_uncert_EKF./lev_arm_base +1.7e-10+(6e-10)/lev_arm_base; % Error in thrust

thrust_mean_EKF(kk) = norm(mean_torque_EKF)/lev_arm_base; % Find thrust

disp("EKF Measured thrust via omega: " + thrust_mean_EKF(kk).*1e6 + " +/- " + thrust_uncert_EKF(kk).*1e6 + " uN")
%}
...
thrust_uncert_UKF(kk) = norm_uncert_UKF./lev_arm_base+1.7e-10+(6e-10)/lev_arm_base; % Error in thrust

thrust_mean_UKF(kk) = norm(mean_torque_UKF)/lev_arm_base; % Find thrust

disp("UKF Measured thrust via omega: " + thrust_mean_UKF(kk).*1e6 + " +/- " + thrust_uncert_UKF(kk).*1e6 + " uN")
end

%% Display and Save Results

thrust_uncert_UKF = thrust_uncert_UKF.*1e6;
thrust_uncert_EKF = thrust_uncert_EKF.*1e6;
disp('== EKF ==')
disp(thrust_mean_EKF.*1e6)
disp(thrust_uncert_EKF)

disp('== UKF ==')
disp(thrust_mean_UKF.*1e6)
disp(thrust_uncert_UKF)
warning('on','all')

save('Optimal_Long_Runs_outKF_Full_Sim')
