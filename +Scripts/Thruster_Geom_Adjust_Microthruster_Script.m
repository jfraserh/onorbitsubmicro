%% Sub-Microthruster Fusion Technique Simulation: Geometry Adjustment
% Jonathan Hood, 1/8/2022
% Simulate an on-orbit sub-microthruster evaluation via the proposed Fusion
% Technique, including orbital dynamics and present perturbations. This
% script adjust the geometry of the spacecraft from 1U to 9U and calculates
% the effect it had on measured thrust.
%% Spacecraft Geometry
...
for kk = 1:9
units = kk;               % number of cubes in configuration
...
for kk = 1:9
    disp("EKF Measured thrust via omega: " + thrust_mean_EKF(kk).*1e6 + "uN")
    disp("UKF Measured thrust via omega: " + thrust_mean_UKF(kk).*1e6 + "uN")
    EKF_perc_reduc = 1-(thrust_mean_EKF(kk)*1e6/control_thrust_EKF);
    UKF_perc_reduc = 1-(thrust_mean_UKF(kk)*1e6/control_thrust_UKF);
    if thrust_mean_EKF(kk)*1e6 < control_thrust_EKF
        fprintf('\n%d U Sat, EKF Results reduced by %f %%\n',kk,EKF_perc_reduc*100)
    end
    if thrust_mean_UKF(kk)*1e6 < control_thrust_UKF
        fprintf('\n%d U Sat, UKF results reduced by %f %%\n',kk,UKF_perc_reduc*100)
    end
end

save('Spacecraft_outKF_Full_Sim_Geom')